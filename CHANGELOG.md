...
## [Alpha 0.2.1-hotfix] - 2026-01-11

### 🐛 紧急修复与优化

- **Web 兼容性修复**：
  - 修复中文字符资源路径 (`v4-embedding-刑法.pak`) 导致的加载失败 (404)，重命名为 ASCII 路径 (`v4-embedding-criminal.pak`)
  - 修复 Web 端字体图标和资源缓存不一致问题
- **UI/UX 改进**：
  - **响应式布局**：修复 `CaseInputScreen` 在宽屏下的组件溢出
  - **溢出保护**：修复 `AnalysisResultScreen` 头部在窄屏/手机端的文字溢出，采用响应式布局策略
  - **新图标**：替换应用内部 Shield 图标为官方 `LOGO.png` 并适配圆角背景
- **Android 修复**：
  - 修复 Android 端内置资产路径显示为 `[内置资产]` 的问题，现在显示真实物理路径
